...
public class AirplaneSeatingGUI extends JFrame implements ActionListener {
	...
	private final static String[] RESERVATION_CLASS = {FIRST_CLASS, BUSINESS, COACH};
	...
	private JComboBox cmbTicketType = new JComboBox(RESERVATION_CLASS); // Choice of reservation types
	...
	public AirplaneSeatingGUI(String airplaneFileName) {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Cannot initialize seating chart.", "Error", JOptionPane.ERROR_MESSAGE);
			endExecution();
		}
	}
	/**
		...
		if (e.getSource().equals(btnAdd)) { // Add the new passenger
			...
			try {
				...
				if (ticketClass.equals(FIRST_CLASS))
					newPassenger = new FirstClass(name, (Flight) myFlight, wantsWindow);
				else if (ticketClass.equals(BUSINESS))
					newPassenger = new Business(name, (Flight) myFlight, wantsWindow);
				else if (ticketClass.equals(COACH))
					newPassenger = new Coach(name, (Flight) myFlight, wantsWindow);
